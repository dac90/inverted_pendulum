...
def collect_data(N, data_range=[5, 5, jnp.pi, 20], seed=None):
    ...
    Y_list = []
    for i in range(N):
        cartpole.setState(X_4dim[i])
        cartpole.performAction(0.0)
        y = cartpole.getState() - X_4dim[i]
        Y_list.append(y)
    Y = jnp.stack(Y_list)
    # Compute std per column

    noise_scale = 0.1 * jnp.std(Y, axis=0)

    key, subkey = random.split(key)
    noise = random.normal(subkey, shape=Y.shape)

    noisy_Y = Y + (noise * noise_scale)
    axis_names_X = ['Cart Location (m)', 'Cart Velocity (m/s)', 'Sin Pole Angle', 'Cos Pole Angle', 'Pole Velocity (rad/s)']
    
    return X_5dim, noisy_Y, axis_names_X
...
def non_linear_rollout(initial_state, X_basis, alpha, sigma, end_time=10.0, dt=0.1):
    ...
    def step_fn(carry, _):
        state, time = carry  
        state_5_dim = conv_4_to_5_single(state)
        d_state = vmap(lambda a, s: non_linear_prediction(state_5_dim, X_basis, a, s), in_axes=(1, 0))(alpha, sigma)
