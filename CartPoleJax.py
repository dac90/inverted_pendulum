...
def _loss(state):
    sig = 0.5
    return 1-jnp.exp(-jnp.dot(state,state)/(2.0 * sig**2))
...
class CartPole:
    ...
    def __init__(self, visual=False):
        ...
        self.pole_angle = jnp.pi    # angle is defined to be zero when the pole is upright, pi when hanging vertically down
    ...
    def getState(self):
        return jnp.array([self.cart_location,self.cart_velocity,self.pole_angle,self.pole_velocity])
    ...
    def performAction(self, action = 0.0):
        ...
        for step in range(self.sim_steps):
            s = jnp.sin(self.pole_angle)
            c = jnp.cos(self.pole_angle)
   ...
   # the following are graphics routines
    def drawPlot(self):
        ...
        # draw pole
        self.pole = Line2D([self.cart_location, self.cart_location + jnp.sin(self.pole_angle)], 
                           [0, jnp.cos(self.pole_angle)], linewidth=3, color='black')
    ...
    def _render(self):
        ...
        self.pole.set_xdata([self.cart_location, self.cart_location + jnp.sin(self.pole_angle)])
        self.pole.set_ydata([0, jnp.cos(self.pole_angle)])
        self.fig.show()
        #plt.pause(0.2)
